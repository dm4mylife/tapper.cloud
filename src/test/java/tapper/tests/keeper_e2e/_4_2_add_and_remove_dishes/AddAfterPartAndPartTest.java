...
class AddAfterPartAndPartTest extends BaseTest {
    ...
    @Test
    @Order(1)
    @DisplayName("Создание заказа в r_keeper открытие стола, проверка что позиции на кассе совпадают с позициями в таппере")
    ...
    @Test
    @Order(9)
    @DisplayName("Закрываем заказ, проверяем тг")
    void closeOrder() {
